...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(empty($eventName_err)){
        ...
        if (isset($_POST['start'])) {
            $sqlUpdate = "update event set startdate = null where name = ?";
            if( ($stmt2 = mysqli_prepare($link, $sqlUpdate))){
                ...
                if(mysqli_stmt_execute($stmt2) ){
                    echo "<script>alert('Start date has been removed for ".$param_eventname.".')</script>";     
                }else{
                    echo "<script>alert('Oops! Something went wrong. Please try again later.')</script>";     
                }
        ...
        elseif (isset($_POST['end'])) {
            $sqlUpdate = "update event set enddate = null where name = ?";
            if( ($stmt2 = mysqli_prepare($link, $sqlUpdate))){
                // Bind variables to the prepared statement as parameters
                ...
                //Attempt to execute the prepared statement
                if(mysqli_stmt_execute($stmt2) ){
                    echo "<script>alert('End date has been removed for ".$param_eventname.".')</script>";     
                }else{
                    echo "<script>alert('Oops! Something went wrong. Please try again later.')</script>";     
                }
...
<head>
    <meta charset="UTF-8">
    <title>Remove Date of Event</title>
...
?>
	...
	<div class="form-group">
    	<input type="submit" class = "btn btn-danger" name="start" value="Delete Start Date">
		<input type="submit" class = "btn btn-danger" name="end" value="Delete End Date">
    </div>
    </form>
